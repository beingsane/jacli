...
class JacliModelDeploy extends JModelBase
{
    ...
    private function setup()
    {
        ...
        {
            if('1' != $this->application->input->get('updaterepo'))
            {
        ...
        else
        {
            if($browserBin)
            {
                ...
                foreach($links as $linkTitle => $linkUrl)
                {
                    ...
                    $command = '' //'DISPLAY=:0 XAUTHORITY=/home/jtester/.Xauthority '
                        .$browserBin.' "'.$httpBase.'/'.$target.$linkUrl.'" &';
